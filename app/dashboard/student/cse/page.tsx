...
export default function StudentCsePage() {
  const subjects = DB.getQuizzes().filter((q) => q.branch === "cse");

  return (
    <RoleGuard allowedRoles={["student"]} requiredBranch="cse">
      <div className="container mx-auto p-4">
        <DashboardHeader />
        <h1 className="text-2xl font-bold mb-2">CSE Student Dashboard</h1>
        <p className="text-sm text-muted-foreground mb-4">Quick overview and suggested quizzes for CSE.</p>
        <DashboardCards />
        <SubjectList branch="cse" />
